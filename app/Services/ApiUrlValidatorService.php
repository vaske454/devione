...
class ApiUrlValidatorService
{
    ...
    public function isValidApiUrl(string $apiUrl): bool
    {
        ...
        if (empty($apiUrl)) {
            throw new InvalidArgumentException('API URL is empty.', 400);
        }
        ...
        if ($apiUrl !== $expectedUrl) {
            throw new InvalidArgumentException('Invalid API URL.', 400);
        }

        // Check if the URL has a valid format
        if (!filter_var($apiUrl, FILTER_VALIDATE_URL)) {
            throw new InvalidArgumentException('Invalid URL format.', 400);
        }
